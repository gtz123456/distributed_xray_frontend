...
export default function AdminVouchers({
  ...
  const fetchVouchers = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || ""}/admin/vouchers${params}`,
        {
  ...
  const revoke = async (code: string) => {
    await fetch(
      `${process.env.NEXT_PUBLIC_API_URL || ""}/admin/voucher/${code}`,
      {
        method: "DELETE",
        headers: { REGKEY: regkey },
      },
  ...
  const createVoucher = async () => {
    await fetch(
      `${process.env.NEXT_PUBLIC_API_URL || ""}/admin/generatevoucher`,
      {
        method: "POST",
  ...
  return (
    <div>
      ...
      {creating && (
        <Modal
          title={dict.createVoucherModal.title}
