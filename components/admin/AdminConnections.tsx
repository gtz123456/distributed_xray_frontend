...
export default function AdminConnections({
  ...
  const fetchConns = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || ""}/admin/connections`,
        {
  ...
  const kick = async (uuid: string) => {
    await fetch(
      `${process.env.NEXT_PUBLIC_API_URL || ""}/admin/user/${uuid}/disconnect`,
      {
        method: "POST",
        headers: { REGKEY: regkey },
      },
          ...
          <table className="w-full">
            <thead>
              <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
                {[d.email, d.node, d.port, d.lastSeen, d.disconnect].map(
                  (h) => (
                    <th
                      key={h}
