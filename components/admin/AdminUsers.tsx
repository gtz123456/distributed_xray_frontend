...
export default function AdminUsers({
  ...
  const [setPlanVal, setSetPlanVal] = useState({
    plan: "Premium plan",
    action: "add",
    delta_amount: 1,
    delta_unit: "months",
  ...
  const btnClass =
    "px-2 py-1 rounded-lg text-xs font-medium transition-all hover:opacity-80";
  ...
  return (
    <div>
            ...
            {filtered.map((u) => (
              ...
              >
                ...
                <td className={tdClass}>
                  ...
                  >
                    {formatBytes(u.traffic_used)} /{" "}
                    {u.traffic_limit === -1
                      ? "∞"
                      : formatBytes(u.traffic_limit)}
                  </div>
                  {u.traffic_limit > 0 && (
                    <div
                    ...
                    >
                      <div
                        ...
                        style={{
                          ...
                          background:
                            u.traffic_used / u.traffic_limit > 0.8
                              ? "#f87171"
                              : "#7c3aed",
                        }}
                      />
                ...
                >
                  {new Date(u.plan_end).toLocaleDateString(undefined, {
                    timeZone: "UTC",
                  })}
                </td>
                <td className={tdClass}>
                  <div className="flex gap-1 flex-wrap">
                    ...
                    <button
                      ...
                      onClick={() =>
                        action(`/admin/user/${u.uuid}/resettraffic`)
                      }
                    >
                      {d.resetTraffic}
                    </button>
                    <button
                      ...
                      onClick={() => {
                        if (confirm(d.confirmBan))
                          action(`/admin/user/${u.uuid}/ban`);
                      }}
                    >
                      {d.ban}
                    </button>
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (confirm(d.confirmDelete))
                          action(`/admin/user/${u.uuid}`, "DELETE");
                      }}
                    >
                      {d.delete}
                    </button>
      ...
      {setPlanUser && (
        <Modal
          title={dict.setPlanModal.title}
          onClose={() => setSetPlanUser(null)}
        >
          <div className="flex flex-col gap-3">
            <select
              ...
              onChange={(e) =>
                setSetPlanVal((v) => ({ ...v, plan: e.target.value }))
              }
            >
              <option value="Free plan">Free plan</option>
              <option value="Premium plan">Premium plan</option>
            ...
            <select
              ...
              onChange={(e) =>
                setSetPlanVal((v) => ({ ...v, action: e.target.value }))
              }
            >
              <option value="add">
                {dict.setPlanModal.actionModify || "Modify Duration"}
              </option>
              <option value="set">
                {dict.setPlanModal.actionSet || "Set Exact Date"}
              </option>
            </select>
            ...
            {setPlanVal.action === "add" ? (
              <div className="flex gap-2">
                <input
                  ...
                  onChange={(e) =>
                    setSetPlanVal((v) => ({
                      ...v,
                      delta_amount: Number(e.target.value),
                    }))
                  }
                />
                <select
                  ...
                  onChange={(e) =>
                    setSetPlanVal((v) => ({ ...v, delta_unit: e.target.value }))
                  }
                >
                  <option value="days">
                    {dict.setPlanModal.unitDays || "Days"}
                  </option>
                  <option value="months">
                    {dict.setPlanModal.unitMonths || "Months"}
                  </option>
                  <option value="years">
                    {dict.setPlanModal.unitYears || "Years"}
                  </option>
                </select>
              </div>
            ) : (
              <input
                ...
                onChange={(e) =>
                  setSetPlanVal((v) => ({ ...v, end_date: e.target.value }))
                }
              />
            )}
            <div className="flex gap-2 justify-end">
              ...
              <button
                ...
                onClick={async () => {
                  await action(
                    `/admin/user/${setPlanUser.uuid}/setplan`,
                    "POST",
                    setPlanVal,
                  );
                  setSetPlanUser(null);
      ...
      {balanceUser && (
        <Modal
          title={dict.addBalanceModal.title}
        ...
        >
          <div className="flex flex-col gap-3">
            <input
              ...
              onChange={(e) =>
                setBalanceVal((v) => ({ ...v, amount: Number(e.target.value) }))
              }
            />
            <input
              className="rounded-lg px-3 py-2 text-sm text-white"
              ...
              onChange={(e) =>
                setBalanceVal((v) => ({ ...v, note: e.target.value }))
              }
            />
            <div className="flex gap-2 justify-end">
              <button
              ...
              <button
                ...
                onClick={async () => {
                  await action(
                    `/admin/user/${balanceUser.uuid}/addbalance`,
                    "POST",
                    {
                      ...balanceVal,
